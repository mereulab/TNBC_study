...
ggsave(plot = plot, filename = file.path(savePlots, "UMAP_sample_split_by_annotation.pdf"), width = 18, height = 15)
...
markers_harmony_clusters <- FindAllMarkers(data, only.pos = T)

data@meta.data <- data@meta.data %>% mutate(annotation = case_when(harmony_clusters == "0" ~ "ER-neg",
                                                                   harmony_clusters == "1" ~ "ER-pos",
                                                                   harmony_clusters == "2" ~ "Basal",
                                                                   harmony_clusters == "3" ~ "Unclass",
                                                                   harmony_clusters == "4" ~ "Unclass",
                                                                   ))

plot <- DimPlot(data, group.by = "annotation", label = T, reduction = "umap.harmony")
ggsave(plot, filename = file.path(savePlots, "UMAP_annotation.pdf"))
